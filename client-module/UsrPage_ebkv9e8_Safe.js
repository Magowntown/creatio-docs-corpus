define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/[]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/()/**SCHEMA_ARGS*/ {
	return {
		...
		handlers: /**SCHEMA_HANDLERS*/[
			// SAFE VERSION - Defensive data loading with guaranteed pass-through
			// Only modifies specific lookup data sources, never blocks other requests
			{
				request: "crt.LoadDataRequest",
				handler: async (request, next) => {
					// ALWAYS call next first to ensure page business rules work
					// Then apply our filters only to specific known data sources

					const knownDataSources = [
						"LookupAttribute_nt0mer7_List_DS",  // Sales Group
						"LookupAttribute_bsixu8a_List_DS"   // Reports
					];

					const dsName = request.dataSourceName || request.customSource || "";

					// If this is NOT one of our target data sources, pass through immediately
					if (!knownDataSources.includes(dsName)) {
						return await next?.handle(request);
					}

					// Apply filters only to our known data sources
					try {
						...
						// Sales Group: filter by BGIsActive = true
						if (dsName === "LookupAttribute_nt0mer7_List_DS") {
						...
						// Reports: filter by UsrActive = true
						if (dsName === "LookupAttribute_bsixu8a_List_DS") {
							request.parameters = addOrReplaceFilterParam(request.parameters, {
								items: {
									"UsrActiveOnly": {
										filterType: 1,
						...
						// If filter application fails, log and continue without filter
						console.warn("[UsrPage_ebkv9e8] Filter application failed:", filterError);
					}
