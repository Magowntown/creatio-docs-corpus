/**
 * UsrPage_ebkv9e8 - v19.6 Customer Lookup
 * Package: BGApp_eykaguu
 *
 * BASED ON: Working v19.1 (PROD)
 *
 * v19.6 FIXES (2026-01-27):
 * - FIX Issue 3: Customer filter - proper lookup using Entity_e7ac661.Customer column
 * - FIX Issue 2: Sales Group cascade - filters by YearMonth to show only active groups
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // Customer - uses page entity column (NEW - user added this)
                    "UsrCustomer": {
                        "modelConfig": {
                            "path": "UsrEntity_e7ac661DS.Customer"
                        }
                    },
                    "UsrCustomer_List": {
                        "isCollection": true,
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19.6] Page init - Customer lookup + cascade filter support");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    // Only intercept SalesGroup list when cascade is enabled
                    if (cascadeFilterEnabled && dsName.includes("SalesGroup") && dsName.includes("_List")) {
                        console.log("[v19.6] Intercepting Sales Group load:", dsName);
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v19.6] Applied filter for", validSalesGroupIds.length, "groups");
                            ...
                            console.log("[v19.6] No valid groups - showing empty");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19.6] Error checking report URL:", e);
                            }
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19.6] COMMISSION report - cascade filter ENABLED");
                                ...
                                cascadeFilterEnabled = false;
                                console.log("[v19.6] ITEMS BY CUSTOMER report - customer filter VISIBLE");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.6] LOOKER report");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.6] EXCEL report");
                            }
                        } else {
                            request.$context.UsrShowCommissionFilters = false;
                            request.$context.UsrShowDateStatusFilters = false;
                    ...
                    // YearMonth change - fetch valid sales groups for cascade
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v19.6] YearMonth changed:", yearMonth.displayValue);
                            request.$context.UsrSalesGroup = null;

                            // Query commission data to find valid sales groups
                            try {
                                // Query BGCommissionReportDataView for sales groups with data
                                const queryUrl = "/0/odata/BGCommissionReportDataView?" +
                                    "$filter=BGYearMonth/Id eq " + yearMonth.value +
                                    "&$apply=groupby((BGSalesRep/BGSalesGroupLookup))&$top=100";

                                console.log("[v19.6] Querying valid sales groups...");
                                ...
                                if (resp.ok) {
                                    ...
                                    (data.value || []).forEach(row => {
                                        // Extract sales group ID from the nested structure
                                        const sgLookup = row.BGSalesRep?.BGSalesGroupLookup;
                                        if (sgLookup) {
                                            groupIds.add(sgLookup);
                                        }
                                    });

                                    validSalesGroupIds = Array.from(groupIds);
                                    console.log("[v19.6] Found", validSalesGroupIds.length, "valid sales groups");

                                    if (validSalesGroupIds.length === 0) {
                                        // Fallback: try simpler query
                                        console.log("[v19.6] Trying fallback query...");
                                        ...
                                        if (fbResp.ok) {
                                            ...
                                            const fbGroups = new Set();
                                            (fbData.value || []).forEach(row => {
                                                if (row.BGSalesGroup) fbGroups.add(row.BGSalesGroup);
                                            });
                                            validSalesGroupIds = Array.from(fbGroups);
                                            console.log("[v19.6] Fallback found", validSalesGroupIds.length, "groups");
                                        }
                                    }
                                } else {
                                    console.log("[v19.6] Query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v19.6] Error fetching sales groups:", e);
                                validSalesGroupIds = null;
                            }
                        } else {
                            validSalesGroupIds = null;
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19.6] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[v19.6] Opening Looker:", fullUrl);
                    ...
                    // EXCEL PATH
                    console.log("[v19.6] Generating Excel:", reportDisplayName);
                    ...
                    if (reportDisplayName.toLowerCase().includes("items by customer")) {
                        try {
                            ...
                            if (customer && customer.value) {
                                ...
                                console.log("[v19.6] Customer:", customer.displayValue, "| ID:", customerId);
                            }
                        } catch (e) {}
                    }
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19.6] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19.6] Service response:", result);
                        ...
                        console.error("[v19.6] Error:", error);
