/**
 * UsrPage_ebkv9e8 - v19.11 Clear Cache Fix
 * Package: BGApp_eykaguu
 *
 * FIXES:
 * - Issue 2: Sales Group CASCADE FILTER - CACHE CLEARING FIX
 *            v19.9/v19.10 bug: UsrSalesGroup_List was cached, not reloaded after switching reports
 *            v19.11 fix: Clear UsrSalesGroup_List when report changes OR YearMonth changes
 *                        This forces Freedom UI to make a fresh LoadDataRequest
 * - Issue 3: Customer filter for "Items by Customer" report (unchanged)
 *
 * KEY INSIGHT:
 * The problem is that Freedom UI caches the dropdown list in UsrSalesGroup_List.
 * The crt.LoadDataRequest interceptor only fires when loading NEW data.
 * If the list is cached, opening the dropdown shows cached (stale) data.
 *
 * THE FIX:
 * 1. When report changes: Clear UsrSalesGroup_List = null
 * 2. When YearMonth changes: Clear UsrSalesGroup_List = null (BEFORE and AFTER cascade query)
 * 3. This forces a fresh LoadDataRequest when dropdown is opened
 * 4. The interceptor applies the filter with current validSalesGroupIds
 */
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19.11] Page init - Cache clearing fix");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v19.11] LoadDataRequest for Sales Group | validIds:",
                            validSalesGroupIds === null ? "null (no filter)" :
                            validSalesGroupIds.length === 0 ? "[] (no matches - show all)" :
                            validSalesGroupIds.length + " groups to filter");

                        // Only apply filter if we have actual IDs
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v19.11] ✓ Applied cascade filter for", validSalesGroupIds.length, "groups");
                        } else {
                            console.log("[v19.11] No filter applied - showing all groups");
                        }
                    }
            ...
            // ================================================================
            // REPORT SELECTION - Visibility + cascade state + CACHE CLEARING
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19.11] Error checking report URL:", e);
                            }
                            ...
                            // v19.11 KEY FIX: Clear the Sales Group list cache!
                            // This forces a fresh LoadDataRequest when dropdown is opened
                            try {
                                request.$context.UsrSalesGroup_List = null;
                                console.log("[v19.11] ✓ Cleared UsrSalesGroup_List cache");
                            } catch (e) {
                                console.log("[v19.11] Could not clear list cache:", e);
                            }
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19.11] COMMISSION report | Cascade: ENABLED");
                                ...
                                cascadeFilterEnabled = false;
                                console.log("[v19.11] ITEMS BY CUSTOMER report");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.11] LOOKER report");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.11] OTHER EXCEL report");
                            }
                        } else {
                            request.$context.UsrShowCommissionFilters = false;
                            request.$context.UsrShowDateStatusFilters = false;
                            ...
                            cascadeFilterEnabled = false;
                            validSalesGroupIds = null;
                            console.log("[v19.11] No report selected");
                        }
                    }

                    // --------------------------------------------------------
                    // YEARMONTH CHANGE - Fetch valid Sales Groups + CLEAR CACHE
                    // --------------------------------------------------------
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        // v19.11: IMMEDIATELY clear the list cache before doing anything
                        try {
                            request.$context.UsrSalesGroup_List = null;
                            console.log("[v19.11] ✓ Cleared UsrSalesGroup_List (YearMonth change)");
                        } catch (e) {
                            console.log("[v19.11] Could not clear list:", e);
                        }
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v19.11] YearMonth changed to:", yearMonth.displayValue);
                            ...
                            try {
                                ...
                                console.log("[v19.11] Fetching cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v19.11] ✓ Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v19.11] No groups found - will show all");
                                    }
                                } else {
                                    console.log("[v19.11] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v19.11] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // v19.11: Clear list cache AGAIN after query completes
                            // This ensures any load that happened during query is invalidated
                            try {
                                request.$context.UsrSalesGroup_List = null;
                                console.log("[v19.11] ✓ Cleared list cache after cascade query");
                            } catch (e) {}

                        } else {
                            validSalesGroupIds = null;
                            console.log("[v19.11] YearMonth cleared - no filter");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19.11] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[v19.11] Opening Looker:", reportDisplayName);
                    ...
                    // EXCEL PATH
                    console.log("[v19.11] Generating Excel:", reportDisplayName);
                    ...
                    if (reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            ...
                            if (yearMonth && yearMonth.value) {
                                ...
                                console.log("[v19.11] Commission YearMonth:", yearMonth.displayValue);
                            }
                        } catch (e) {}
                        try {
                            ...
                            if (salesGroup && salesGroup.value) {
                                ...
                                console.log("[v19.11] Commission SalesGroup:", salesGroup.displayValue);
                            }
                        } catch (e) {}
                    }
                    ...
                    if (reportDisplayName.toLowerCase().includes("items by customer")) {
                        try {
                            ...
                            if (customer && customer.value) {
                                ...
                                console.log("[v19.11] Customer:", customer.displayValue);
                            }
                        } catch (e) {}
                    }
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19.11] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19.11] Service response:", result);
                        ...
                        console.error("[v19.11] Error:", error);
