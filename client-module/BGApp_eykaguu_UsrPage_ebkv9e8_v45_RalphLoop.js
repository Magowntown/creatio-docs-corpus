/**
 * UsrPage_ebkv9e8 - v45 RALPH LOOP SOLUTION
 * Package: BGApp_eykaguu
 *
 * COMPREHENSIVE FIX combining ALL learnings:
 *
 * 1. LOOKER REPORTS: Embedded in parent's UsrIframe (user's v41 fix)
 *    - GridContainer_fh039aq visibility bound to $UsrShowLookerFrame
 *    - setUsrIframeUrl() sets parent's UsrIframe.Url property
 *
 * 2. COMMISSION FILTERS: YearMonth + SalesGroup lookups (v19.13 pattern)
 *    - Bound to existing page data source: UsrEntity_e7ac661DS.BGYearMonth
 *    - Cascade filter: Sales Group filtered by YearMonth via HandlerChainService reload
 *
 * 3. CUSTOMER FILTER: Account entity lookup (embeddedModel pattern)
 *    - UsrCustomer_List uses embeddedModel with Account entity
 *    - Populates ComboBox from Account records
 *
 * 4. DYNAMIC VISIBILITY: Container-based binding
 *    - Containers bound to visibility attributes
 *    - Controls inside containers always visible
 *
 * BASED ON:
 * - v19.13: Working YearMonth/SalesGroup pattern + cascade filter
 * - v41: User's UsrIframe fix for Looker
 * - Creatio Academy: embeddedModel pattern for external entity lookups
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v45] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v45] UsrIframe element not found - trying attribute binding");
                }
            } catch (e) {
                console.log("[v45] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // Customer ComboBox - LOOKUP TO ACCOUNT ENTITY
            {
                "operation": "insert",
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // ================================================================
                    // CUSTOMER LOOKUP (embeddedModel pattern for Account entity)
                    // This is the key pattern for external entity lookups!
                    // ================================================================
                    "UsrCustomer": {
                        "modelConfig": {
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v45] Page initialized - Ralph Loop Solution");
                    console.log("[v45] Features: Looker iframe + Commission lookups + Customer Account lookup");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v45] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v45] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
            ...
            // ================================================================
            // REPORT SELECTION - Update visibility + cascade state
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v45] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v45] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v45] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v45] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v45] -> ITEMS BY CUSTOMER: Customer lookup + Date filters");
                            ...
                            console.log("[v45] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v45] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v45] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v45] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v45] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v45] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v45] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v45] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v45] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v45] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v45] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v45] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v45] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v45] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v45] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v45] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v45] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v45] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v45] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (cust && cust.value) {
                                ...
                                console.log("[v45] Customer:", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v45] WARNING: No customer selected");
                            }
                        } catch (e) {
                            console.log("[v45] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v45] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v45] Generation failed:", errorMsg);
