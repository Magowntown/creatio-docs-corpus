/**
 * UsrPage_ebkv9e8 - v49 HYBRID CUSTOMER (ComboBox + Fallback Dialog)
 * Package: BGApp_eykaguu
 *
 * STRATEGY: Try embeddedModel ComboBox first, with fallback to lookup dialog if needed.
 *
 * This version combines:
 * 1. embeddedModel pattern from Creatio Academy for Customer ComboBox (primary)
 * 2. crt.OpenLookupPageRequest as fallback (if ComboBox doesn't populate)
 * 3. v19.13 cascade filter pattern for YearMonth/SalesGroup (working)
 * 4. User's v41 iframe fix for Looker Studio (working)
 *
 * KEY INSIGHT FROM RESEARCH:
 * - Freedom UI officially supports only ONE data source per page
 * - embeddedModel is documented but may have edge cases
 * - The ComboBox's control attribute and _List attribute must follow naming convention
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v49] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v49] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v49] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
    ...
    // Fallback customer selection (if ComboBox doesn't work)
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // Primary: ComboBox with embeddedModel
            // Fallback: Button opens lookup dialog if ComboBox empty
            // ================================================================
            {
                "operation": "insert",
            ...
            // Customer ComboBox - LOOKUP using embeddedModel pattern
            {
                "operation": "insert",
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // ================================================================
                    // CUSTOMER LOOKUP - embeddedModel pattern from Creatio Academy
                    // The ComboBox control binds to UsrCustomer
                    // The dropdown list is populated from UsrCustomer_List via embeddedModel
                    // ================================================================
                    "UsrCustomer": {
                        "value": null
                    },
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v49] Page initialized - Hybrid Customer approach");
                    console.log("[v49] Features: embeddedModel ComboBox + fallback dialog + cascade filters + Looker iframe");
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            if (!list || (Array.isArray(list) && list.length === 0)) {
                                console.log("[v49] Customer list empty - enabling fallback button");
                                ctx.UsrShowCustomerFallback = true;
                            } else {
                                console.log("[v49] Customer list has items - ComboBox should work");
                            }
                        } catch (e) {
                            console.log("[v49] Could not check customer list:", e);
                            ctx.UsrShowCustomerFallback = true;
                        }
                    }, 2000);
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v49] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v49] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    const ctx = request.$context;
                    console.log("[v49] Opening Account lookup dialog (fallback)...");
                        ...
                        await ctx.executeRequest({
                            ...
                            afterClosed: function(selectedItems) {
                                if (selectedItems && Object.keys(selectedItems).length > 0) {
                                    ...
                                    console.log("[v49] Fallback customer selected:", fallbackCustomerName, "| ID:", fallbackCustomerId);
                                    Terrasoft.showInformation("Customer selected: " + fallbackCustomerName);
                                } else {
                                    console.log("[v49] No customer selected (dialog closed)");
                                }
                            }
                        });
                    } catch (e) {
                        console.error("[v49] Error opening lookup dialog:", e);
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v49] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v49] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v49] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v49] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v49] -> ITEMS BY CUSTOMER: Customer ComboBox + Date filters");
                            ...
                            setTimeout(async () => {
                                try {
                                    const list = await ctx.UsrCustomer_List;
                                    if (!list || (Array.isArray(list) && list.length === 0)) {
                                        console.log("[v49] Customer list empty after report selection - showing fallback");
                                        ctx.UsrShowCustomerFallback = true;
                                    }
                                } catch (e) {
                                    console.log("[v49] Customer list check error:", e);
                                    ctx.UsrShowCustomerFallback = true;
                                }
                            }, 1000);
                            ...
                            console.log("[v49] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v49] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrCustomer" && !request.silent) {
                        ...
                        if (customer && customer.value) {
                            console.log("[v49] Customer selected via ComboBox:", customer.displayValue, "| ID:", customer.value);
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v49] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v49] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v49] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v49] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v49] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v49] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v49] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v49] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v49] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v49] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v49] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v49] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v49] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v49] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v49] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v49] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v49] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (cust && cust.value) {
                                ...
                                console.log("[v49] Customer from ComboBox:", customerName, "| ID:", customerId);
                            } else if (fallbackCustomerId) {
                                // Use fallback from dialog
                                customerId = fallbackCustomerId;
                                customerName = fallbackCustomerName;
                                console.log("[v49] Customer from fallback dialog:", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v49] WARNING: No customer selected");
                            ...
                            if (fallbackCustomerId) {
                                customerId = fallbackCustomerId;
                                customerName = fallbackCustomerName;
                                console.log("[v49] Customer from fallback (after error):", customerName);
                            } else {
                                console.log("[v49] Customer lookup error:", e);
                    ...
                    try {
                        ...
                        console.log("[v49] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v49] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v49] Error:", e);
