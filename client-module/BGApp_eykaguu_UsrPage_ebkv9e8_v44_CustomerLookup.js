/**
 * UsrPage_ebkv9e8 - v44 CUSTOMER LOOKUP
 ...
 *
 * CUSTOMER IS A LOOKUP TO ACCOUNT ENTITY
 * Added Account data source and proper lookup binding
 */
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v44] UsrIframe.Url set:", url);
                } else {
                    console.log("[v44] WARNING: UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v44] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
        ...
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // Customer COMBOBOX - LOOKUP TO ACCOUNT ENTITY
            {
                ...
                "values": {
                    ...
                    "visible": true,
                    "layoutConfig": { "column": 1, "row": 1, "colSpan": 1, "rowSpan": 1 },
                    "tooltip": "Select customer from Account"
                },
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // Customer - LOOKUP TO ACCOUNT ENTITY
                    "UsrCustomer": {
                        "modelConfig": {
                    ...
                    // Customer list from Account data source
                    "UsrCustomer_List": {
                        "isCollection": true,
                        "modelConfig": {
                            "path": "UsrCustomerDS",
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            // Page init
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v44] Page initialized - Account lookup for Customer");
                    return;
                }
            },
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v44] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v44] No report selected");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v44] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v44] -> Commission filters ON");

                        } else if (reportName.includes("items by customer")) {
                            ctx.UsrShowCustomerFilter = true;
                            ctx.UsrShowDateFilters = true;
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v44] -> Customer LOOKUP + Date filters ON");
                            ...
                            console.log("[v44] -> Looker iframe ENABLED:", reportUrl);

                        } else {
                            ctx.UsrShowDateFilters = true;
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v44] -> Date filters ON (Excel report)");
                        }
                    }
            ...
            // Generate report
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v44] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v44] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v44] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v44] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "NULL/EMPTY");
                        } else {
                    ...
                    if (!intEsq || intEsq.trim() === "") {
                        console.warn("[v44] WARNING: IntEsq is empty - backend may fail");
                    }
                    ...
                    console.log("[v44] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        ...
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v44] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v44] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}

                    } else if (rLower.includes("items by customer")) {
                        // Get Customer LOOKUP value - displayValue is the Account.Name
                        try {
                            const cust = await ctx.UsrCustomer;
                            if (cust && cust.value) {
                                customerName = cust.displayValue || "";
                                console.log("[v44] Customer selected:", customerName, "| ID:", cust.value);
                            } else {
                                console.log("[v44] WARNING: No customer selected");
                            }
                        } catch (e) {
                            console.log("[v44] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v44] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v44] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v44] Error:", e);
                        Terrasoft.showErrorMessage("Error: " + e.message);
                    }
