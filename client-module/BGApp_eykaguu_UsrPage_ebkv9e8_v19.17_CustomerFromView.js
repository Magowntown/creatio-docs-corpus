/**
 * UsrPage_ebkv9e8 - v19.17 Customer From View Fix
 * Package: BGApp_eykaguu
 *
 * CHANGES FROM v19.16:
 * - FIXED: Customer dropdown now populates from DISTINCT BGCustomer values in BGSalesByItemView
 * - This ensures dropdown shows EXACT customer names that exist in the view
 * - Removes dependency on Account entity lookup (which may have different names)
 *
 * ROOT CAUSE OF "No data found":
 * - v19.16 bound Customer dropdown to Account entity via UsrEntity_e7ac661DS.BGCustomer
 * - User selected "Pampa Bay" (company account) which doesn't exist in BGSalesByItemView
 * - BGSalesByItemView has customers like: "Bay Country Shop", "Baytree Gift Company", etc.
 * - Name mismatch caused 0 results
 *
 * INCLUDES ALL v19.16 FEATURES:
 * - Date filters (CreatedFrom/To, ShippingFrom/To, DeliveryFrom/To) with sync attribute access
...
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function buildLookerParams(context) {
        ...
        // Use SAME pattern as parent schema (BGlobalLookerStudio)
    ...
    // ================================================================
    // CUSTOMER DROPDOWN STATE (for Items by Customer)
    // Populated from DISTINCT BGCustomer values in BGSalesByItemView
    // ================================================================
    var customerOptions = [];  // [{value: "Customer Name", displayValue: "Customer Name"}, ...]
        ...
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            },
            {
                ...
                "values": {
                    ...
                    "layoutConfig": { "column": 1, "row": 1, "colSpan": 1, "rowSpan": 1 },
                    "visible": true,
                    "tooltip": "Customers from BGSalesByItemView"
                },
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // YearMonth - bound to page entity (REQUIRED for dropdown to work)
                    "UsrYearMonth": {
                        "modelConfig": {
                            "path": "UsrEntity_e7ac661DS.BGYearMonth"
                        }
                    },
                    // SalesGroup - bound to page entity (REQUIRED for dropdown to work)
                    "UsrSalesGroup": {
                        "modelConfig": {
                    ...
                    // Customer Name - simple string attribute (not a lookup)
                    // Populated from DISTINCT BGCustomer values in BGSalesByItemView
                    "UsrCustomerName": {
                        "value": null
                    },
                    // Customer options list - populated dynamically
                    "UsrCustomerName_List": {
                        "isCollection": true,
                        "viewModelConfig": {
                            "attributes": {
                                "value": { "type": "crt.String" },
                                "displayValue": { "type": "crt.String" }
                            }
                        }
                    }
                }
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19.17] Page init - Customer from BGSalesByItemView");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v19.17] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v19.17] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        } else {
                            console.log("[v19.17] No cascade filter - showing all groups");
                        }
                    }

                    // Intercept Customer list for "Items by Customer"
                    const isCustomerList = dsName.includes("CustomerName") && dsName.includes("_List");
                    if (isCustomerList && customerOptions.length > 0) {
                        console.log("[v19.17] Returning", customerOptions.length, "customers from BGSalesByItemView");
                        // Return custom data instead of entity query
                        return {
            ...
            // ================================================================
            // REPORT SELECTION - Visibility + cascade state + customer loading
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19.17] Error checking report URL:", e);
                            }
                            ...
                            request.$context.UsrCustomerName = null;
                            validSalesGroupIds = null;
                            customerOptions = [];
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19.17] COMMISSION report | Cascade: ENABLED");
                                ...
                                cascadeFilterEnabled = false;
                                console.log("[v19.17] ITEMS BY CUSTOMER report | Loading customers from view...");

                                // Load customer options from BGSalesByItemView
                                try {
                                    // Query DISTINCT BGCustomer values from the view
                                    // Use OData $apply=groupby to get distinct values
                                    const custUrl = "/0/odata/BGSalesByItemView?$apply=groupby((BGCustomer))&$orderby=BGCustomer&$top=500";
                                    console.log("[v19.17] Querying customers:", custUrl);
                                    ...
                                    if (custResp.ok) {
                                        ...
                                        const customers = (custData.value || [])
                                        ...
                                        console.log("[v19.17] Loaded", customerOptions.length, "customers from BGSalesByItemView");
                                        if (customerOptions.length > 0) {
                                            console.log("[v19.17] First 5:", customerOptions.slice(0, 5).map(c => c.displayValue));
                                        }
                                    } else {
                                        console.log("[v19.17] Customer query failed:", custResp.status);
                                    }
                                } catch (e) {
                                    console.log("[v19.17] Customer query error:", e);
                                }
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.17] LOOKER report | Filters: Date+Status");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.17] OTHER EXCEL report | Filters: Date+Status");
                            }
                        } else {
                            ...
                            validSalesGroupIds = null;
                            customerOptions = [];
                            console.log("[v19.17] No report selected | All filters hidden");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v19.17] YearMonth changed to:", yearMonth.displayValue);
                            ...
                            try {
                                ...
                                console.log("[v19.17] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v19.17] Found", validSalesGroupIds.length, "groups with data for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v19.17] No groups found for", yearMonth.displayValue, "- will show ALL groups");
                                    }
                                } else {
                                    console.log("[v19.17] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v19.17] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown
                            try {
                                console.log("[v19.17] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v19.17] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v19.17] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            request.$context.UsrSalesGroup = null;
                            console.log("[v19.17] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19.17] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[v19.17] Opening Looker:", reportDisplayName);
                    ...
                    // EXCEL PATH
                    // --------------------------------------------------------
                    console.log("[v19.17] Generating Excel:", reportDisplayName);
                    ...
                    if (reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            ...
                            if (yearMonth && yearMonth.value) {
                                ...
                                console.log("[v19.17] Commission YearMonth:", yearMonth.displayValue);
                            }
                        } catch (e) {}
                        try {
                            ...
                            if (salesGroup && salesGroup.value) {
                                ...
                                console.log("[v19.17] Commission SalesGroup:", salesGroup.displayValue);
                            }
                        } catch (e) {}
                    }

                    // Get Customer filter value for "Items by Customer"
                    // NOTE: UsrCustomerName is a simple string, not a lookup
                    if (reportDisplayName.toLowerCase().includes("items by customer")) {
                        try {
                            const custName = await context.UsrCustomerName;
                            if (custName) {
                                // Handle both string value and object with displayValue
                                if (typeof custName === 'string') {
                                    customerName = custName;
                                } else if (custName.displayValue) {
                                    customerName = custName.displayValue;
                                } else if (custName.value) {
                                    customerName = custName.value;
                                }
                                console.log("[v19.17] Customer filter:", customerName);
                            }
                        } catch (e) {
                            console.log("[v19.17] Error getting customer:", e);
                        }
                    }

                    // Get Date and Status filters (for "Items by Customer" and similar reports)
                    ...
                    // Only collect these filters for non-Commission reports
                    if (!reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            const attrs = request.$context.attributes;

                            console.log("[v19.17] Raw date attributes:", {
                                CreatedFrom: attrs.CreatedFrom,
                                CreatedTo: attrs.CreatedTo,
                                ShippingFrom: attrs.ShippingFrom,
                                ShippingTo: attrs.ShippingTo,
                                DeliveryFrom: attrs.DeliveryFrom,
                                DeliveryTo: attrs.DeliveryTo
                            });

                            // Convert to WCF format /Date(ms)/ for backend
                            if (attrs.CreatedFrom != null) {
                                createdFrom = formatDateForWcf(attrs.CreatedFrom);
                            }
                            if (attrs.CreatedTo != null) {
                                createdTo = formatDateForWcf(attrs.CreatedTo);
                            }
                            if (attrs.ShippingFrom != null) {
                                shippingFrom = formatDateForWcf(attrs.ShippingFrom);
                            }
                            if (attrs.ShippingTo != null) {
                                shippingTo = formatDateForWcf(attrs.ShippingTo);
                            }
                            if (attrs.DeliveryFrom != null) {
                                deliveryFrom = formatDateForWcf(attrs.DeliveryFrom);
                            }
                            if (attrs.DeliveryTo != null) {
                                deliveryTo = formatDateForWcf(attrs.DeliveryTo);
                            }
                            ...
                            console.log("[v19.17] Converted filters:", {
                                createdFrom, createdTo, shippingFrom, shippingTo,
                                deliveryFrom, deliveryTo, statusName, customerName
                            });
                        } catch (e) {
                            console.log("[v19.17] Error getting date/status filters:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19.17] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19.17] Service response:", result);
                        ...
                        console.error("[v19.17] Error:", error);
