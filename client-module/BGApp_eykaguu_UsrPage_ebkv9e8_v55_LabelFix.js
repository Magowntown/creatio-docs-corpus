/**
 * UsrPage_ebkv9e8 - v55 LABEL FIX
 * Package: BGApp_eykaguu
 *
 * v55 FIX:
 * - Fixed "Report false" label to just "Report"
 *
 * v54 FIX:
 * - Freedom UI lookup dialog returns FLAT object, not nested map:
 *   { value: "guid", displayValue: "name" }
 * - Extract directly: selectedItems.value and selectedItems.displayValue
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v54] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v54] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v54] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // FIX: Report lookup label - remove "false" from label
            {
                "operation": "merge",
                "name": "LookupAttribute_0as4io2",
                "values": { "label": "Report" }
            },
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v54] Page initialized - Lookup Dialog approach");
                    console.log("[v54] Features: Looker iframe + Commission lookups + Customer via lookup dialog");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v54] Opening Account lookup dialog...");
                    ...
                    try {
                        await ctx.executeRequest({
                            ...
                            afterClosed: function(selectedItems) {
                                // Freedom UI returns FLAT object: { value: "guid", displayValue: "name" }
                                console.log("[v54] afterClosed selectedItems:", selectedItems);

                                if (selectedItems && selectedItems.value) {
                                    // Extract directly from flat object
                                    const extractedId = selectedItems.value;
                                    const extractedName = selectedItems.displayValue || "Unknown";

                                    // Validate GUID
                                    const guidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

                                    if (guidRegex.test(extractedId)) {
                                        ...
                                        console.log("[v54] SUCCESS - Customer:", selectedCustomerName, "| ID:", selectedCustomerId);
                                    } else {
                                        console.log("[v54] ERROR: Invalid GUID:", extractedId);
                                    }
                                } else {
                                    console.log("[v54] No customer selected (dialog closed or no value)");
                                }
                            }
                        });
                    } catch (e) {
                        console.error("[v54] Error opening lookup dialog:", e);
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v54] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v54] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v54] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v54] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v54] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v54] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v54] -> ITEMS BY CUSTOMER: Customer button + Date filters");
                            ...
                            console.log("[v54] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v54] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v54] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v54] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v54] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v54] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v54] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v54] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v54] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v54] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v54] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v54] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v54] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v54] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v54] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v54] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v54] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v54] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v54] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            console.log("[v54] Customer attr raw:", custIdAttr, "| extracted:", custId);
                            ...
                            if (isValidGuid) {
                                ...
                                console.log("[v54] Customer:", customerName, "| ID:", customerId);
                            } else if (selectedCustomerId && guidRegex.test(selectedCustomerId)) {
                                // Fallback to handler state variable
                                customerId = selectedCustomerId;
                                customerName = selectedCustomerName;
                                console.log("[v54] Customer (from state):", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v54] WARNING: No valid customer selected");
                                console.log("[v54] custIdAttr:", custIdAttr, "| selectedCustomerId:", selectedCustomerId);
                                Terrasoft.showErrorMessage("Please click 'Select Customer' button and choose a customer first");
                                return next?.handle(request);
                            }
                        } catch (e) {
                            console.log("[v54] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v54] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v54] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v54] Error:", e);
