/**
 * UsrPage_ebkv9e8 - v43 CUSTOMER TEXT
 * Package: BGApp_eykaguu
 *
 * BUILDS ON USER'S v41 MODIFICATIONS:
 * - Uses parent's UsrIframe component (setUsrIframeUrl function)
 * - UsrShowLookerFrame visibility for GridContainer_fh039aq
 * - ComboBox lookups for YearMonth and SalesGroup
 *
 * FIX: Customer changed to TEXT INPUT
 * Reason: BGCustomer in BGSalesByItemView is VARCHAR, not a lookup entity.
...
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v43] UsrIframe.Url set:", url);
                } else {
                    console.log("[v43] WARNING: UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v43] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            // Parent iframe container - show only for Looker reports (USER'S v41 FIX)
            {
                "operation": "merge",
            ...
            // FIX: TEXT INPUT because BGCustomer is VARCHAR, not entity lookup
            // ================================================================
            {
                "operation": "insert",
            ...
            // Customer TEXT INPUT (NOT ComboBox - BGCustomer is varchar in the view)
            {
                "operation": "insert",
                ...
                "values": {
                    ...
                    "visible": true,
                    "layoutConfig": { "column": 1, "row": 1, "colSpan": 1, "rowSpan": 1 },
                    "tooltip": "Searches BGCustomer column using CONTAINS"
                },
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // Customer name - TEXT (BGCustomer is varchar, not entity)
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            // Page init
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v43] Page initialized - Lookups + Text Customer + Looker iframe");
                    return;
                }
            },
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v43] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v43] No report selected");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v43] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v43] -> Commission filters ON");

                        } else if (reportName.includes("items by customer")) {
                            ctx.UsrShowCustomerFilter = true;
                            ctx.UsrShowDateFilters = true;
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v43] -> Customer text input + Date filters ON");

                        } else if (reportUrl) {
                            // LOOKER: Show parent iframe and set URL
                            ctx.UsrShowDateFilters = true;
                            ctx.UsrShowLookerFrame = true;
                            ctx.UsrLookerUrl = reportUrl;
                            setUsrIframeUrl(reportUrl);
                            console.log("[v43] -> Looker iframe ENABLED:", reportUrl);

                        } else {
                            ctx.UsrShowDateFilters = true;
                            ctx.UsrShowLookerFrame = false;
                            console.log("[v43] -> Date filters ON (Excel report)");
                        }
                    }
            ...
            // Generate report
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v43] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v43] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v43] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v43] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "NULL/EMPTY");
                        } else {
                    ...
                    if (!intEsq || intEsq.trim() === "") {
                        console.warn("[v43] WARNING: IntEsq is empty - backend may fail");
                    }
                    ...
                    console.log("[v43] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        ...
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v43] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v43] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}

                    } else if (rLower.includes("items by customer")) {
                        // Get Customer TEXT value
                        try {
                            customerName = (await ctx.UsrCustomerName) || "";
                            console.log("[v43] CustomerName (text):", customerName);
                        } catch (e) {
                            console.log("[v43] CustomerName error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v43] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v43] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v43] Error:", e);
                        Terrasoft.showErrorMessage("Error: " + e.message);
                    }
