/**
 * UsrPage_ebkv9e8 - v19 Looker Fix
 * Package: BGApp_eykaguu
 *
 * CHANGES FROM v18:
 * - FIX: Looker reports now SHOW Date+Status filters (was incorrectly hidden)
 * - FIX: Looker reports now BUILD URL params from filters (was just opening URL)
 * - Original parent behavior restored for Looker reports
 *
 * v19.1 FIXES (2026-01-23):
 * - FIX: Added null guard for selectedReport.value in attribute change handler
 * - FIX: Added null guard for meta.UsrURL (OData response can be malformed)
 * - FIX: Added null guard for reportMeta in generate handler
 *
 * VISIBILITY RULES:
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19] Page init - visibility controlled by attribute binding");
                    console.log("[v19] UsrShowCommissionFilters:", request.$context.UsrShowCommissionFilters);
                    console.log("[v19] UsrShowDateStatusFilters:", request.$context.UsrShowDateStatusFilters);
                    return;
                }
            },
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19] Error checking report URL:", e);
                            }
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19] Report:", selectedReport.displayValue, "| Type: COMMISSION | Showing commission filters");
                                ...
                                console.log("[v19] Report:", selectedReport.displayValue, "| Type: LOOKER | Showing date+status filters");

                            } else {
                                // NON-COMMISSION EXCEL: Show date+status, hide commission filters
                                request.$context.UsrShowCommissionFilters = false;
                                request.$context.UsrShowDateStatusFilters = true;
                                console.log("[v19] Report:", selectedReport.displayValue, "| Type: EXCEL | Showing date+status filters");
                            }
                        } else {
                            // NO REPORT SELECTED: Hide everything
                            request.$context.UsrShowCommissionFilters = false;
                            request.$context.UsrShowDateStatusFilters = false;
                            console.log("[v19] No report selected | All filters hidden");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19] Metadata lookup failed:", e);
                    }
                    ...
                    // EXCEL PATH
                    console.log("[v19] Generating Excel report:", reportDisplayName);
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19] Excel service response:", result);
