define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
	return {
		...
		handlers: /**SCHEMA_HANDLERS*/[
			...
			// Report generation handler - uses custom UsrExcelReportService
			{
				request: "usr.GenerateExcelReportRequest",
				handler: async (request, next) => {
					...
					// Call custom UsrExcelReportService
					try {
						...
						const response = await fetch("/0/rest/UsrExcelReportService/Generate", {
							...
							body: JSON.stringify({
								...
								YearMonthId: yearMonthId !== emptyGuid ? yearMonthId : emptyGuid,
								SalesGroupId: salesGroupId !== emptyGuid ? salesGroupId : emptyGuid
							})
						});
						...
						if (result.success && result.key) {
							const downloadUrl = "/0/rest/UsrExcelReportService/GetReport/" + result.key;
							...
							const errorMsg = result.message || result.errorMessage || result.errorInfo?.message || "Unknown error";
