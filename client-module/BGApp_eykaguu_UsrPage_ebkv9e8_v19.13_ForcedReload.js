/**
 * UsrPage_ebkv9e8 - v19.13 Forced Reload
 * Package: BGApp_eykaguu
 *
 * COMBINED FIX from v19.9 + v19.3:
 * - Uses modelConfig.path (v19.9) - WORKING dropdown binding
 * - Uses sdk.HandlerChainService.instance.process() (v19.3) - FORCES dropdown reload after cascade query
 *
 * ROOT CAUSE OF v19.9/v19.10/v19.11/v19.12 FAILURES:
 * - v19.9: No force reload - cached dropdown list wasn't refreshed after cascade query
 * - v19.10: Added readonly state - didn't address caching
 * - v19.11: Tried clearing list attribute - didn't trigger reload
 * - v19.12: Used lookupListConfig only - broke dropdown binding entirely
 *
 * THE FIX: After cascade query completes, use sdk.HandlerChainService to process
 * a crt.LoadDataRequest with loadType: "reload" to force the dropdown to refresh.
 *
 * VISIBILITY RULES:
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19.13] Page init - Forced Reload fix enabled");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v19.13] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v19.13] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        } else {
                            // No filter to apply - show all groups (graceful fallback)
                            console.log("[v19.13] No cascade filter - showing all groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19.13] Error checking report URL:", e);
                            }
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19.13] COMMISSION report | Cascade: ENABLED | Filters: YearMonth+SalesGroup");
                                ...
                                cascadeFilterEnabled = false;
                                console.log("[v19.13] ITEMS BY CUSTOMER report | Filters: Date+Status+Customer");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.13] LOOKER report | Filters: Date+Status");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.13] OTHER EXCEL report | Filters: Date+Status");
                            }
                        } else {
                            ...
                            cascadeFilterEnabled = false;
                            validSalesGroupIds = null;
                            console.log("[v19.13] No report selected | All filters hidden");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v19.13] YearMonth changed to:", yearMonth.displayValue);
                            ...
                            try {
                                ...
                                console.log("[v19.13] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v19.13] Found", validSalesGroupIds.length, "groups with data for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v19.13] No groups found for", yearMonth.displayValue, "- will show ALL groups");
                                    }
                                } else {
                                    console.log("[v19.13] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;  // Error case - show all
                                }
                            } catch (e) {
                                console.log("[v19.13] Cascade query error:", e);
                                validSalesGroupIds = null;  // Error case - show all
                            }
                            ...
                            // ============================================================
                            try {
                                console.log("[v19.13] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v19.13] Sales Group dropdown reloaded successfully");
                            } catch (reloadError) {
                                console.log("[v19.13] Sales Group reload error (non-critical):", reloadError);
                            ...
                            validSalesGroupIds = null;
                            request.$context.UsrSalesGroup = null;
                            console.log("[v19.13] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19.13] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[v19.13] Opening Looker:", reportDisplayName);
                    ...
                    // EXCEL PATH
                    // --------------------------------------------------------
                    console.log("[v19.13] Generating Excel:", reportDisplayName);
                    ...
                    if (reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            ...
                            if (yearMonth && yearMonth.value) {
                                ...
                                console.log("[v19.13] Commission YearMonth:", yearMonth.displayValue);
                            }
                        } catch (e) {}
                        try {
                            ...
                            if (salesGroup && salesGroup.value) {
                                ...
                                console.log("[v19.13] Commission SalesGroup:", salesGroup.displayValue);
                            }
                        } catch (e) {}
                    }
                    ...
                    if (reportDisplayName.toLowerCase().includes("items by customer")) {
                        try {
                            ...
                            if (customer && customer.value) {
                                ...
                                console.log("[v19.13] Customer filter:", customerName, "| ID:", customerId);
                            }
                        } catch (e) {}
                    }
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19.13] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19.13] Service response:", result);
                        ...
                        console.error("[v19.13] Error:", error);
