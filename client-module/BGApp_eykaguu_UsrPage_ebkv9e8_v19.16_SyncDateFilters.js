/**
 * UsrPage_ebkv9e8 - v19.16 Sync Date Filters Fix (Parent Pattern)
 * Package: BGApp_eykaguu
 *
 * CHANGES FROM v19.15:
 * - FIXED: Date filters now use SAME pattern as parent schema (BGlobalLookerStudio)
 * - Parent uses: request.$context.attributes.CreatedFrom (direct synchronous access)
 * - Added debug logging to show raw date values before conversion
 * - Uses != null check (like parent) instead of truthy check
 *
 * CHANGES FROM v19.14:
 * - Passes date filters (CreatedFrom/To, ShippingFrom/To, DeliveryFrom/To) to backend
 * - Passes StatusName filter to backend
 * - Backend applies these filters to BGSalesByItemView for "Items by Customer"
 *
 * INCLUDES ALL v19.14 FIXES:
 * - Passes CustomerName (displayValue) in addition to CustomerId to backend
 * - Enables filtering on BGSalesByItemView which only has BGCustomer (varchar name)
 *
 * INCLUDES ALL v19.13 FIXES:
 * - Uses modelConfig.path (v19.9) - WORKING dropdown binding
 * - Uses sdk.HandlerChainService.instance.process() (v19.3) - FORCES dropdown reload
 *
 * VISIBILITY RULES:
 ...
 * | Non-Commission Excel | Hidden            | VISIBLE     | Hidden   | -       |
...
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function buildLookerParams(context) {
        ...
        // Parent uses: request.$context.attributes.CreatedFrom (direct sync access)
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v19.16] Page init - Forced Reload fix enabled");
            ...
            // KEY: Only apply filter when validSalesGroupIds has items
            //      If empty array, DON'T filter (show all groups - graceful fallback)
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v19.16] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v19.16] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        } else {
                            // No filter to apply - show all groups (graceful fallback)
                            console.log("[v19.16] No cascade filter - showing all groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue && selectedReport.value) {
                            ...
                            try {
                                ...
                                console.log("[v19.16] Error checking report URL:", e);
                            }
                            ...
                            if (isCommissionReport) {
                                ...
                                console.log("[v19.16] COMMISSION report | Cascade: ENABLED | Filters: YearMonth+SalesGroup");
                                ...
                                cascadeFilterEnabled = false;
                                console.log("[v19.16] ITEMS BY CUSTOMER report | Filters: Date+Status+Customer");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.16] LOOKER report | Filters: Date+Status");
                                ...
                                request.$context.UsrShowCustomerFilter = false;
                                cascadeFilterEnabled = false;
                                console.log("[v19.16] OTHER EXCEL report | Filters: Date+Status");
                            }
                        } else {
                            ...
                            cascadeFilterEnabled = false;
                            validSalesGroupIds = null;
                            console.log("[v19.16] No report selected | All filters hidden");
                        }
                    }

                    // --------------------------------------------------------
                    // YEARMONTH CHANGE - Fetch valid Sales Groups (cascade)
                    // KEY ADDITION: Force reload Sales Group dropdown after query
                    // --------------------------------------------------------
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v19.16] YearMonth changed to:", yearMonth.displayValue);
                            ...
                            try {
                                ...
                                console.log("[v19.16] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v19.16] Found", validSalesGroupIds.length, "groups with data for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v19.16] No groups found for", yearMonth.displayValue, "- will show ALL groups");
                                    }
                                } else {
                                    console.log("[v19.16] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;  // Error case - show all
                                }
                            } catch (e) {
                                console.log("[v19.16] Cascade query error:", e);
                                validSalesGroupIds = null;  // Error case - show all
                            }

                            // ============================================================
                            // KEY FIX: Force reload Sales Group dropdown using HandlerChainService
                            // This is from v19.3 - ensures cached dropdown list is refreshed
                            // ============================================================
                            try {
                                console.log("[v19.16] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v19.16] Sales Group dropdown reloaded successfully");
                            } catch (reloadError) {
                                console.log("[v19.16] Sales Group reload error (non-critical):", reloadError);
                            ...
                            validSalesGroupIds = null;
                            request.$context.UsrSalesGroup = null;
                            console.log("[v19.16] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v19.16] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[v19.16] Opening Looker:", reportDisplayName);
                    ...
                    // EXCEL PATH
                    // --------------------------------------------------------
                    console.log("[v19.16] Generating Excel:", reportDisplayName);
                    ...
                    if (reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            ...
                            if (yearMonth && yearMonth.value) {
                                ...
                                console.log("[v19.16] Commission YearMonth:", yearMonth.displayValue);
                            }
                        } catch (e) {}
                        try {
                            ...
                            if (salesGroup && salesGroup.value) {
                                ...
                                console.log("[v19.16] Commission SalesGroup:", salesGroup.displayValue);
                            }
                        } catch (e) {}
                    }
                    ...
                    if (reportDisplayName.toLowerCase().includes("items by customer")) {
                        try {
                            ...
                            if (customer && customer.value) {
                                ...
                                console.log("[v19.16] Customer filter:", customerName, "| ID:", customerId);
                            }
                        } catch (e) {}
                    }
                    ...
                    // Only collect these filters for non-Commission reports that show Date+Status filters
                    if (!reportDisplayName.toLowerCase().includes("commission")) {
                        try {
                            // Date filters - use SAME pattern as parent schema (BGlobalLookerStudio)
                            // Parent uses: request.$context.attributes.CreatedFrom (direct sync access)
                            const attrs = request.$context.attributes;

                            // Debug: Log what we have
                            console.log("[v19.16] Raw date attributes:", {
                            ...
                            // Status filter - lookup attribute
                            const status = attrs.LookupAttribute_tytkx09;
                            if (status && status.displayValue && status.displayValue !== "All") {
                                statusName = status.displayValue;
                            }

                            console.log("[v19.16] Converted date filters:", {
                                createdFrom, createdTo, shippingFrom, shippingTo,
                                deliveryFrom, deliveryTo, statusName
                            });
                        } catch (e) {
                            console.log("[v19.16] Error getting date/status filters:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v19.16] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v19.16] Service response:", result);
                        ...
                        console.error("[v19.16] Error:", error);
