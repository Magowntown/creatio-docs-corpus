/**
 * UsrPage_ebkv9e8 - v15 Iframe Handler
 * Package: BGApp_eykaguu
 *
 * CHANGES FROM v14:
 * - Looker reports now display in IFRAME (not new tab)
 * - Iframe container shown/hidden based on report type
 * - Filter visibility verified for all report types
 *
 * VISIBILITY RULES:
 * | Report Type          | Commission Filters | Date+Status Filters | Iframe |
 * |----------------------|-------------------|---------------------|--------|
 * | None selected        | Hidden            | Hidden              | Hidden |
 * | Commission           | VISIBLE           | Hidden              | Hidden |
 * | Non-Commission Excel | Hidden            | VISIBLE             | Hidden |
 * | Looker Studio        | Hidden            | Hidden              | VISIBLE|
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
     ...
     * @param {string} filterGroup - "dateStatus" or "all"
     * @param {boolean} visible - true to show, false to hide
     */
    function toggleDateStatusFilters(visible) {
        ...
        console.log("[v15] Date+Status filters:", visible ? "VISIBLE" : "HIDDEN");
    }

    /**
     * Toggle iframe container visibility and set URL
     * @param {boolean} visible - true to show iframe
     * @param {string} url - URL to load in iframe (optional)
     */
    function toggleIframe(visible, url) {
        ...
        // Find the actual iframe element and set src
        if (visible && url) {
            ...
            if (!iframe) {
                ...
                iframe = document.querySelector('[data-item-marker="UsrIframe"] iframe');
            }
            if (!iframe) {
                iframe = document.querySelector('.usr-iframe iframe');
            }
            if (!iframe) {
                // Last resort - find any iframe in the page
                var allIframes = document.querySelectorAll('iframe');
                for (var i = 0; i < allIframes.length; i++) {
                    if (allIframes[i].id !== 'reportDownloadFrame') {
                        iframe = allIframes[i];
                        break;
                    }
                }
            ...
            if (iframe) {
                iframe.src = url;
                console.log("[v15] Iframe src set to:", url);
            } else {
                console.log("[v15] Iframe element not found, opening in new tab");
                window.open(url, "_blank");
            }
        }

        console.log("[v15] Iframe container:", visible ? "VISIBLE" : "HIDDEN");
    }
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // ================================================================
            // IFRAME CONTAINER - Keep visible, we control via DOM
            // (Removed the visible:false so iframe can be shown)
            // ================================================================
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    setTimeout(function() {
                        console.log("[v15] Page init - setting default visibility");
                        toggleDateStatusFilters(false);  // Hide date+status
                        toggleIframe(false);              // Hide iframe
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue) {
                            ...
                            try {
                                ...
                                console.log("[v15] Error checking report URL:", e);
                            }
                            ...
                            if (isLookerReport) {
                                // LOOKER STUDIO: Show iframe, hide all filters
                                request.$context.UsrShowCommissionFilters = false;
                                toggleDateStatusFilters(false);
                                toggleIframe(true, reportUrl);
                                console.log("[v15] Report:", selectedReport.displayValue, "| Type: LOOKER | Showing iframe");

                            } else if (isCommissionReport) {
                                // COMMISSION: Show commission filters, hide date+status, hide iframe
                                request.$context.UsrShowCommissionFilters = true;
                                toggleDateStatusFilters(false);
                                toggleIframe(false);
                                console.log("[v15] Report:", selectedReport.displayValue, "| Type: COMMISSION | Showing commission filters");

                            } else {
                                // NON-COMMISSION EXCEL: Show date+status, hide commission filters, hide iframe
                                request.$context.UsrShowCommissionFilters = false;
                                toggleDateStatusFilters(true);
                                toggleIframe(false);
                                console.log("[v15] Report:", selectedReport.displayValue, "| Type: EXCEL | Showing date+status filters");
                            }
                        } else {
                            ...
                            toggleDateStatusFilters(false);
                            toggleIframe(false);
                            console.log("[v15] No report selected | All filters hidden");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[v15] Metadata lookup failed:", e);
                    }

                    // LOOKER STUDIO - Iframe should already be showing, just refresh
                    if (reportUrl && reportUrl.length > 0) {
                        toggleIframe(true, reportUrl);
                        Terrasoft.showInformation("Report loaded in iframe");
                        console.log("[v15] Looker report refreshed in iframe");
                        return next?.handle(request);
                    }

                    // EXCEL PATH
                    console.log("[v15] Generating Excel report:", reportDisplayName);
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v15] Found template:", odataResult.value[0].IntName);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v15] Excel service response:", result);
