/**
 * UsrPage_ebkv9e8 - v53 LOOKUP DEBUG
 * Package: BGApp_eykaguu
 *
 * v53 DIAGNOSTICS:
 * - Full JSON logging of selectedItems structure from lookup dialog
 * - Try multiple property paths for customer ID/Name extraction
 * - GUID regex validation on all potential ID sources
 *
 * Based on v52 with enhanced debugging for lookup dialog callback
 */
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v53] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v53] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v53] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v53] Page initialized - Lookup Dialog approach");
                    console.log("[v53] Features: Looker iframe + Commission lookups + Customer via lookup dialog");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v53] Opening Account lookup dialog...");
                    ...
                    try {
                        await ctx.executeRequest({
                            ...
                            afterClosed: function(selectedItems) {
                                // FULL DEBUG: Log entire selectedItems structure
                                console.log("[v53] afterClosed raw selectedItems:", selectedItems);
                                try {
                                    console.log("[v53] selectedItems JSON:", JSON.stringify(selectedItems, null, 2));
                                } catch (e) {
                                    console.log("[v53] Could not stringify selectedItems:", e.message);
                                }

                                if (selectedItems && Object.keys(selectedItems).length > 0) {
                                    const keys = Object.keys(selectedItems);
                                    console.log("[v53] Keys:", keys);
                                    const firstKey = keys[0];
                                    const selected = selectedItems[firstKey];

                                    console.log("[v53] firstKey:", firstKey, "| type:", typeof firstKey);
                                    console.log("[v53] selected object:", selected);

                                    // GUID regex for validation
                                    ...
                                    // Path 1: firstKey IS the GUID (typical Creatio lookup result)
                                    if (guidRegex.test(firstKey)) {
                                        extractedId = firstKey;
                                        console.log("[v53] ID from firstKey (GUID):", extractedId);
                                    }

                                    // Path 2: selected.Id
                                    if (!extractedId && selected && selected.Id && guidRegex.test(selected.Id)) {
                                        extractedId = selected.Id;
                                        console.log("[v53] ID from selected.Id:", extractedId);
                                    }

                                    // Path 3: selected.value (might be GUID)
                                    if (!extractedId && selected && selected.value && guidRegex.test(selected.value)) {
                                        extractedId = selected.value;
                                        console.log("[v53] ID from selected.value:", extractedId);
                                    }

                                    // Path 4: selected.$id or selected.id
                                    if (!extractedId && selected) {
                                        const altId = selected.$id || selected.id || selected.ID;
                                        if (altId && guidRegex.test(altId)) {
                                            extractedId = altId;
                                            console.log("[v53] ID from alternate property:", extractedId);
                                        }
                                    }

                                    // Extract name - try multiple paths
                                    if (selected) {
                                        extractedName = selected.Name || selected.displayValue || selected.name ||
                                                       selected.DisplayValue || selected.Caption || firstKey || "Unknown";
                                    }

                                    console.log("[v53] FINAL: ID=", extractedId, "| Name=", extractedName);

                                    if (extractedId) {
                                        selectedCustomerId = extractedId;
                                        selectedCustomerName = extractedName;
                                        ...
                                        console.log("[v53] SUCCESS - Customer:", selectedCustomerName, "| ID:", selectedCustomerId);
                                    } else {
                                        console.log("[v53] ERROR: Could not extract valid GUID from selection");
                                        console.log("[v53] Available properties:", selected ? Object.keys(selected) : "null");
                                    }
                                } else {
                                    console.log("[v53] No customer selected (dialog closed or empty)");
                                }
                            }
                        });
                    } catch (e) {
                        console.error("[v53] Error opening lookup dialog:", e);
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v53] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v53] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v53] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v53] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v53] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v53] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v53] -> ITEMS BY CUSTOMER: Customer button + Date filters");
                            ...
                            console.log("[v53] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v53] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v53] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v53] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v53] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v53] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v53] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v53] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v53] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v53] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v53] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v53] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v53] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v53] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v53] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v53] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v53] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v53] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v53] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            console.log("[v53] Customer attr raw:", custIdAttr, "| extracted:", custId);
                            ...
                            if (isValidGuid) {
                                ...
                                console.log("[v53] Customer:", customerName, "| ID:", customerId);
                            } else if (selectedCustomerId && guidRegex.test(selectedCustomerId)) {
                                // Fallback to handler state variable
                                customerId = selectedCustomerId;
                                customerName = selectedCustomerName;
                                console.log("[v53] Customer (from state):", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v53] WARNING: No valid customer selected");
                                console.log("[v53] custIdAttr:", custIdAttr, "| selectedCustomerId:", selectedCustomerId);
                                Terrasoft.showErrorMessage("Please click 'Select Customer' button and choose a customer first");
                                return next?.handle(request);
                            }
                        } catch (e) {
                            console.log("[v53] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v53] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v53] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v53] Error:", e);
