/**
 * UsrPage_ebkv9e8 - v48 EMBEDDED MODEL (Creatio Academy Pattern)
 * Package: BGApp_eykaguu
 *
 * SOLUTION: Use embeddedModel pattern from Creatio Academy documentation
 ...
 *
 * This provides a PROPER ComboBox lookup for Customer (Account entity)
 * without needing a separate data source in modelConfigDiff.
 *
 * KEY PATTERN (from Creatio Academy):
 * "LookupAttributeCityList": {
 *     "isCollection": true,
 *     "modelConfigDiff": { "path": "EmbeddedDS" },
 *     "embeddedModel": {
 *         "name": "EmbeddedDS",
 *         "config": {
 *             "type": "crt.EntityDataSource",
 *             "config": { "entitySchemaName": "City" }
 *         }
 *     },
 *     "viewModelConfigDiff": {
 *         "attributes": {
 *             "value": { "modelConfigDiff": { "path": "EmbeddedDS.Id" } },
 *             "displayValue": { "modelConfigDiff": { "path": "EmbeddedDS.Name" } }
 *         }
 *     }
 * }
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v48] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v48] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v48] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
    ...
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // Uses COMBOBOX with embeddedModel pattern from Creatio Academy
            // ================================================================
            {
                "operation": "insert",
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "values": {
                    ...
                    // This is the CORRECT way to bind ComboBox to external entity!
                    // ================================================================
                    "UsrCustomer": {
                        "value": null
                    },
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v48] Page initialized - Embedded Model approach (Creatio Academy pattern)");
                    console.log("[v48] Features: Looker iframe + Commission lookups + Customer via embeddedModel");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v48] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v48] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v48] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v48] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v48] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v48] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v48] -> ITEMS BY CUSTOMER: Customer ComboBox + Date filters");
                            ...
                            console.log("[v48] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v48] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v48] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v48] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v48] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v48] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v48] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v48] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v48] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v48] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v48] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v48] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v48] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v48] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v48] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v48] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v48] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v48] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v48] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}

                    } else if (rLower.includes("items by customer")) {
                        // Get customer from ComboBox (embeddedModel should provide value + displayValue)
                        try {
                            const cust = await ctx.UsrCustomer;
                            if (cust && cust.value) {
                                customerId = cust.value;
                                customerName = cust.displayValue || "";
                                console.log("[v48] Customer:", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v48] WARNING: No customer selected");
                                Terrasoft.showErrorMessage("Please select a customer first");
                                return next?.handle(request);
                            }
                        } catch (e) {
                            console.log("[v48] Customer lookup error:", e);
                    ...
                    try {
                        ...
                        console.log("[v48] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v48] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v48] Error:", e);
