/**
 * UsrPage_ebkv9e8 - v51 STABLE DIALOG (Fix for Infinite Loading)
 * Package: BGApp_eykaguu
 *
 * ROOT CAUSE OF INFINITE LOADING:
 * v50 added modelConfigDiff with "UsrCustomerEmbeddedDS" data source that queries
 * the entire Account entity at page initialization. This caused Freedom UI to hang.
 *
 * FIX: Remove the problematic modelConfigDiff data source declaration.
 * Use Button + Dialog approach for Customer selection instead of ComboBox.
 *
 * Based on v47 (known stable) with:
 ...
 *
 * DEPLOY THIS VERSION TO FIX PROD INFINITE LOADING
 */
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v51] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v51] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v51] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v51] Page initialized - Lookup Dialog approach");
                    console.log("[v51] Features: Looker iframe + Commission lookups + Customer via lookup dialog");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v51] Opening Account lookup dialog...");
                        ...
                        await ctx.executeRequest({
                            ...
                            afterClosed: function(selectedItems) {
                                if (selectedItems && Object.keys(selectedItems).length > 0) {
                                    ...
                                    console.log("[v51] Customer selected:", selectedCustomerName, "| ID:", selectedCustomerId);
                                } else {
                                    console.log("[v51] No customer selected (dialog closed)");
                                }
                            }
                        });
                    } catch (e) {
                        console.error("[v51] Error opening lookup dialog:", e);
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v51] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v51] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v51] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v51] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v51] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v51] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v51] -> ITEMS BY CUSTOMER: Customer button + Date filters");
                            ...
                            console.log("[v51] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v51] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v51] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v51] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v51] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v51] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v51] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v51] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v51] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v51] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v51] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v51] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v51] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v51] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v51] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v51] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v51] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v51] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v51] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (custId) {
                                ...
                                console.log("[v51] Customer:", customerName, "| ID:", customerId);
                            } else if (selectedCustomerId) {
                                // Fallback to handler state
                                customerId = selectedCustomerId;
                                customerName = selectedCustomerName;
                                console.log("[v51] Customer (from state):", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v51] WARNING: No customer selected - use Select Customer button");
                                Terrasoft.showErrorMessage("Please select a customer first");
                                return next?.handle(request);
                            }
                        } catch (e) {
                            console.log("[v51] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v51] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v51] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v51] Error:", e);
