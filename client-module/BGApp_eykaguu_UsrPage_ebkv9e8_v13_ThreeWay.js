/**
 * UsrPage_ebkv9e8 - v13 Three-Way Handler
 * Package: BGApp_eykaguu
 *
 * FIXES v12's issues (client feedback 2026-01-22):
 * - Non-Commission Excel reports need Status filter (was removed)
 * - Non-Commission Excel reports need Date filters (were hidden)
 * - Non-Commission Excel reports need Expand All button
 *
 * VISIBILITY RULES:
 ...
 *
 * APPROACH:
 * - Commission filters: Use $UsrShowCommissionFilters (works - same schema)
 * - Status/Date filters: Use DOM manipulation (cross-schema limitation)
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    return {
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            // ================================================================
            // PAGE INIT - Set initial filter visibility
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    // On page load, hide Commission filters (default state)
                    // Status and date filters from parent will be visible by default
                    setTimeout(() => {
                        toggleParentFilters("none"); // Hide all initially
                        console.log("[v13] Page initialized - no report selected");
                    }, 100);
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue) {
                            ...
                            try {
                                ...
                                console.log("[v13] Error checking report URL:", e);
                            }
                            ...
                            if (hasLookerUrl) {
                                ...
                                toggleParentFilters("none");
                                console.log("[v13] Report:", selectedReport.displayValue, "| Type: Looker Studio | Filters: hidden");
                            } else if (isCommissionReport) {
                                // Commission: Hide parent filters (Commission filters shown via attribute)
                                toggleParentFilters("none");
                                console.log("[v13] Report:", selectedReport.displayValue, "| Type: Commission | Filters: Commission only");
                            } else {
                                // Non-Commission Excel: Show parent filters
                                toggleParentFilters("visible");
                                console.log("[v13] Report:", selectedReport.displayValue, "| Type: Excel | Filters: Status + Dates");
                            }
                        } else {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        if (reportMetaResp.ok) {
                            ...
                            console.log("[v13] Report metadata:", reportDisplayName, "URL:", reportUrl ? "YES" : "NO");
                        }
                    } catch (e) {
                        console.log("[v13] Metadata lookup failed:", e);
                    }

                    // LOOKER STUDIO PATH
                    if (reportUrl && reportUrl.length > 0) {
                        var viewUrl = reportUrl.replace("/embed/u/0/", "/u/0/").replace("/embed/", "/u/0/");
                        console.log("[v13] Opening Looker Studio in new tab:", viewUrl);
                    ...
                    // EXCEL PATH
                    console.log("[v13] No Looker URL, using Excel service for:", reportDisplayName);
                    ...
                    try {
                        ...
                        if (odataResult.value && odataResult.value.length > 0) {
                            ...
                            console.log("[v13] Found IntExcelReport:", odataResult.value[0].IntName, "->", intExcelReportId);
                        } else {
                    ...
                    try {
                        ...
                        console.log("[v13] UsrExcelReportService response:", result);
...
// ================================================================
// HELPER FUNCTIONS (outside schema scope)
// ================================================================

/**
 * Get BPMCSRF cookie for API calls
 */
function getBpmcsrf() {
...
/**
 * Toggle parent filter visibility via DOM
 * @param {string} state - "visible" or "none"
 */
function toggleParentFilters(state) {
    const display = state === "visible" ? "" : "none";

    // Parent filter elements (from BGlobalLookerStudio schema)
    const parentElements = [
        // Status filter container
        "GridContainer_knkow5v",
        // Date filter container
        "GridContainer_xdy25v1",
    ...
    parentElements.forEach(name => {
        const el = document.querySelector('[data-item-marker="' + name + '"]');
        if (el) {
            el.style.display = display;
            console.log("[v13] " + name + " display:", display || "visible");
        }
    });
}
