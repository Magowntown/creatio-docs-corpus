/**
 * UsrPage_ebkv9e8 - v47 LOOKUP DIALOG APPROACH
 * Package: BGApp_eykaguu
 *
 * DISCOVERY from Ralph Loop research:
 * - Freedom UI ComboBox cannot be programmatically populated with custom array data
 * - Virtual objects with IEntityQueryExecutor are complex and require new schema
 * - crt.OpenLookupPageRequest can open Account lookup dialog!
 *
 * SOLUTION: Use a "Select Customer" button that opens Account lookup dialog
 * - Button triggers crt.OpenLookupPageRequest with entitySchemaName: "Account"
 * - afterClosed callback captures selected Account
 * - Display selected customer name in a Label
 *
 * SOURCES:
 ...
 * - https://community.creatio.com/questions/virtual-detail-freedom-ui
 */
    ...
    function setUsrIframeUrl(url) {
        setTimeout(() => {
            try {
                ...
                if (el) {
                    el.Url = url;
                    console.log("[v47] UsrIframe.Url set:", url);
                } else {
                    console.warn("[v47] UsrIframe element not found");
                }
            } catch (e) {
                console.log("[v47] Error setting UsrIframe Url:", e);
            }
        }, 500);
    }
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v47] Page initialized - Lookup Dialog approach");
                    console.log("[v47] Features: Looker iframe + Commission lookups + Customer via lookup dialog");
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v47] Opening Account lookup dialog...");
                        ...
                        await ctx.executeRequest({
                            ...
                            afterClosed: function(selectedItems) {
                                if (selectedItems && Object.keys(selectedItems).length > 0) {
                                    ...
                                    console.log("[v47] Customer selected:", selectedCustomerName, "| ID:", selectedCustomerId);
                                } else {
                                    console.log("[v47] No customer selected (dialog closed)");
                                }
                            }
                        });
                    } catch (e) {
                        console.error("[v47] Error opening lookup dialog:", e);
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    if (cascadeFilterEnabled && isSalesGroupList) {
                        console.log("[v47] Sales Group list load | validIds:",
                        ...
                        if (validSalesGroupIds !== null && validSalesGroupIds.length > 0) {
                            ...
                            console.log("[v47] Applied cascade filter for", validSalesGroupIds.length, "groups");
                        }
                    }
                ...
                handler: async (request, next) => {
                    ...
                    if (request.attributeName === "LookupAttribute_0as4io2" && !request.silent) {
                        ...
                        try {
                            ...
                            console.log("[v47] Error getting report:", e);
                        }
                        ...
                        if (!selectedReport || !selectedReport.value) {
                            console.log("[v47] No report selected - all filters hidden");
                            return next?.handle(request);
                        }

                        const reportName = (selectedReport.displayValue || "").toLowerCase();
                        console.log("[v47] Report selected:", selectedReport.displayValue);
                        ...
                        if (reportName.includes("commission")) {
                            ...
                            console.log("[v47] -> COMMISSION: YearMonth + SalesGroup filters, cascade ENABLED");
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v47] -> ITEMS BY CUSTOMER: Customer button + Date filters");
                            ...
                            console.log("[v47] -> LOOKER: Iframe ENABLED, URL:", reportUrl);
                            ...
                            ctx.UsrShowLookerFrame = false;
                            cascadeFilterEnabled = false;
                            console.log("[v47] -> OTHER EXCEL: Date + Status filters");
                        }
                    }
                    ...
                    if (request.attributeName === "UsrYearMonth" && !request.silent && cascadeFilterEnabled) {
                        ...
                        if (yearMonth && yearMonth.value) {
                            console.log("[v47] YearMonth changed to:", yearMonth.displayValue);
                            ctx.UsrSalesGroup = null;
                            ...
                            try {
                                ...
                                console.log("[v47] Querying cascade data...");
                                ...
                                if (resp.ok) {
                                    ...
                                    if (validSalesGroupIds.length > 0) {
                                        console.log("[v47] Found", validSalesGroupIds.length, "groups for", yearMonth.displayValue);
                                    } else {
                                        console.log("[v47] No groups found - will show ALL groups");
                                    }
                                } else {
                                    console.log("[v47] Cascade query failed:", resp.status);
                                    validSalesGroupIds = null;
                                }
                            } catch (e) {
                                console.log("[v47] Cascade query error:", e);
                                validSalesGroupIds = null;
                            }

                            // Force reload Sales Group dropdown (v19.13 key fix)
                            try {
                                console.log("[v47] Forcing Sales Group dropdown reload...");
                                const reloadRequest = {
                                ...
                                console.log("[v47] Sales Group dropdown reloaded");
                            } catch (reloadError) {
                                console.log("[v47] Sales Group reload error (non-critical):", reloadError);
                            }

                        } else {
                            validSalesGroupIds = null;
                            ctx.UsrSalesGroup = null;
                            console.log("[v47] YearMonth cleared - cascade filter disabled");
                        }
                    }
            ...
            // ================================================================
            {
                ...
                handler: async (request, next) => {
                    ...
                    console.log("[v47] Generate:", reportDisplayName);
                    ...
                    if (reportUrl) {
                        ...
                        try {
                            ...
                            console.log("[v47] Looker params error:", e);
                        }

                        const fullUrl = reportUrl + params;
                        console.log("[v47] Looker URL with params:", fullUrl);
                    ...
                    try {
                        ...
                        if (d && d.value && d.value[0]) {
                            ...
                            console.log("[v47] Template:", d.value[0].IntName, "| IntEsq:", intEsq ? "present" : "EMPTY");
                        } else {
                    ...
                    console.log("[v47] Date filters | From:", dateFrom, "| To:", dateTo, "| Status:", statusName);
                    ...
                    if (rLower.includes("commission")) {
                        try {
                            ...
                            if (ym && ym.value) {
                                ...
                                console.log("[v47] YearMonth:", ym.displayValue, "| ID:", yearMonthId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            ...
                            if (sg && sg.value) {
                                ...
                                console.log("[v47] SalesGroup:", sg.displayValue, "| ID:", salesGroupId);
                            }
                        } catch (e) {}
                        ...
                        try {
                            const custId = await ctx.UsrCustomerId;
                            const custName = await ctx.UsrCustomerName;
                            if (custId) {
                                customerId = custId;
                                customerName = custName || "";
                                console.log("[v47] Customer:", customerName, "| ID:", customerId);
                            } else if (selectedCustomerId) {
                                // Fallback to handler state
                                customerId = selectedCustomerId;
                                customerName = selectedCustomerName;
                                console.log("[v47] Customer (from state):", customerName, "| ID:", customerId);
                            } else {
                                console.log("[v47] WARNING: No customer selected - use Select Customer button");
                                Terrasoft.showErrorMessage("Please select a customer first");
                                return next?.handle(request);
                            }
                        } catch (e) {
                            console.log("[v47] Customer lookup error:", e);
                        }
                    }
                    ...
                    try {
                        ...
                        console.log("[v47] Request body:", JSON.stringify(requestBody, null, 2));
                        ...
                        if (result && result.success && result.key) {
                            ...
                            console.error("[v47] Generation failed:", errorMsg);
                            Terrasoft.showErrorMessage(errorMsg);
                        }
                    } catch (e) {
                        console.error("[v47] Error:", e);
