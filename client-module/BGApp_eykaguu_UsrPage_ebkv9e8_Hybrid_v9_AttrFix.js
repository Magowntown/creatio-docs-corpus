/**
 * UsrPage_ebkv9e8 - Hybrid Handler v9 (Fixed Attribute Declaration)
 *
 * v9 Changes from v7:
 * - FIXED: UsrShowCommissionFilters declared as empty object {} instead of { "value": false }
 * - Initial value set in crt.HandleViewModelInitRequest handler
 * - This matches Creatio Freedom UI documentation pattern for reactive attributes
 *
 * Root cause: Creatio Freedom UI requires attributes used for visibility binding to be
 * declared as empty objects. Setting "value": false in viewModelConfigDiff prevents
 * the UI framework from properly tracking attribute changes.
 */
define("UsrPage_ebkv9e8", /**SCHEMA_DEPS*/["@creatio-devkit/common"]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/(sdk)/**SCHEMA_ARGS*/ {
    return {
        viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
            ...
            // Warning label for Commission reports - CONDITIONAL visibility via attribute binding
            {
                "operation": "insert",
                "name": "BGWarningLabel",
                "values": {
            ...
            // Filter fields container (for Commission reports) - CONDITIONAL visibility
            {
                "operation": "insert",
        ...
        viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
            {
                ...
                "path": ["attributes"],
                "values": {
                    // v9 FIX: Declare as empty object - allows Freedom UI to track changes
                    // Initial value set in HandleViewModelInitRequest handler
        ...
        handlers: /**SCHEMA_HANDLERS*/[
            // v9: Initialize Commission filters visibility on page load
            // CRITICAL: Set initial value here since attribute declared as empty object
            {
                request: "crt.HandleViewModelInitRequest",
                handler: async (request, next) => {
                    // Set initial value - Commission filters hidden by default
                    request.$context.UsrShowCommissionFilters = false;
                    console.log("[UsrPage_ebkv9e8 v9] Page initialized - UsrShowCommissionFilters = false");
                ...
                handler: async (request, next) => {
                    if (request.attributeName === "LookupAttribute_bsixu8a" && !request.silent) {
                        ...
                        if (selectedReport && selectedReport.displayValue) {
                            ...
                            // Clear Commission filter values when changing reports
                            request.$context.LookupAttribute_yubshr1 = null;
                            request.$context.LookupAttribute_nt0mer7 = null;
                        }

                        // Update visibility attribute - should trigger UI update with empty {} declaration
                        request.$context.UsrShowCommissionFilters = isCommissionReport;

                        console.log("[UsrPage_ebkv9e8 v9] Report selected:",
                            selectedReport?.displayValue || "(none)",
                            "| UsrShowCommissionFilters =", isCommissionReport);
                    }
            ...
            // HYBRID Report generation handler
            {
                ...
                handler: async (request, next) => {
                    ...
                    try {
                        ...
                        console.log("[UsrPage_ebkv9e8 v9] Metadata lookup failed:", e);
                    }
                    ...
                    if (reportUrl && reportUrl.length > 0) {
                        ...
                        console.log("[UsrPage_ebkv9e8 v9] Opening Looker Studio:", viewUrl);
                    ...
                    // EXCEL PATH
                    console.log("[UsrPage_ebkv9e8 v9] Using Excel service for:", reportDisplayName);
                    ...
                    try {
                        ...
                        console.log("[UsrPage_ebkv9e8 v9] Service response:", result);
                        ...
                        console.error("[UsrPage_ebkv9e8 v9] Error:", error);
