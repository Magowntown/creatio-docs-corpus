...
> **2026-09-08 ACTIVE (non-Pampa) — Business Central Integration `IWMSDynamicsBCIntegration` = build331 / V156 CURRENT** (`~/creatio-hub/reference/bc-connector/`, README row 50). Live on D4, `mkpdev-2153` and `mkpdev-interweave`. Since 243: portable mappings, Needs-attention work list + record banner, one page grammar on every page, and a Needs-attention tool for the Creatio.ai agent (20 tools). Open: Andrew runs `scripts/cleanup_test_records.py --apply` (classifier-blocked for the agent), then the flows scenario + journey walk; the order document-type default is a decision. Launch readiness 89%; the rest is owner-gated (Entra app, VAT sandbox, SMTP, licensed seat, Marketplace).
>
