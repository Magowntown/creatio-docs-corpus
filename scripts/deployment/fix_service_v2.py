#!/usr/bin/env python3
"""
Fix UsrExcelReportService code via Creatio API - try different endpoints
"""
...
def main():
    ...
    # First try to get the schema via REST API
    print("\n=== Getting schema via REST API ===")
    get_url = f"{CREATIO_URL}/0/rest/SourceCodeDesignerService/GetSchemaData"
    response = session.post(get_url, json={"schemaUId": SCHEMA_UID}, headers=headers, timeout=30)
    print(f"REST GetSchemaData: {response.status_code}")
    ...
    # Try direct save via REST API
    print("\n=== Trying REST SaveSchema ===")
    save_url = f"{CREATIO_URL}/0/rest/SourceCodeDesignerService/SaveSchema"
    save_data = {
        "schemaUId": SCHEMA_UID,
        "body": NEW_CODE
    }
    response = session.post(save_url, json=save_data, headers=headers, timeout=60)
    print(f"REST SaveSchema: {response.status_code}")
    if response.status_code == 200:
        try:
            result = response.json()
            print(f"Result: {json.dumps(result, indent=2)[:500]}")
            if result.get("success") != False:
                print(">>> Success via REST!")
                return True
        except:
            print(f"Response: {response.text[:300]}")

    # Try DataService approach
    print("\n=== Trying DataService UpdateQuery ===")
    ...
    print(f"DataService Update: {response.status_code}")
    if response.status_code == 200:
        try:
            result = response.json()
            print(f"Result: {json.dumps(result, indent=2)[:500]}")
