...
namespace Terrasoft.Configuration
{
    ...
    public class UsrExcelReportService : BaseService
    {
        ...
        /// <summary>
        /// Attempts to retrieve file bytes from various cache locations
        /// </summary>
        private byte[] TryGetFileFromCache(string key)
        {
            // Try session first
            var sessionObj = HttpContext.Current?.Session?[key];
            if (sessionObj != null)
            {
                if (sessionObj is byte[] bytes) return bytes;
                if (sessionObj is MemoryStream ms) return ms.ToArray();
                if (sessionObj is Stream stream)
                {
            ...
            if (cacheObj != null)
            {
                if (cacheObj is byte[] bytes) return bytes;
                if (cacheObj is MemoryStream ms) return ms.ToArray();
                if (cacheObj is Stream stream)
                {
                    using (var memStream = new MemoryStream())
                    {
                        stream.CopyTo(memStream);
            ...
            // Try looking for internal IntExcelExport cache via reflection
            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                foreach (var typeName in new[] {
                    ...
                    "IntExcelExport.Utilities.ReportCache"
                })
                {
                    try
                    {
                        var cacheType = assembly.GetType(typeName);
                        if (cacheType == null) continue;

                        // Try static dictionary fields
                        var fields = cacheType.GetFields(BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Public);
                        foreach (var field in fields)
                        {
                            if (field.FieldType.Name.Contains("Dictionary") || field.FieldType.Name.Contains("Cache"))
                            {
                                var dictValue = field.GetValue(null);
                                if (dictValue is System.Collections.IDictionary dict && dict.Contains(key))
                                {
                                    var item = dict[key];
                                    if (item is byte[] itemBytes) return itemBytes;
                                    if (item is MemoryStream itemMs) return itemMs.ToArray();
                                }
                            }
                        }

                        // Try instance methods
                        var getMethod = cacheType.GetMethod("Get") ?? cacheType.GetMethod("GetFile");
                        if (getMethod != null && getMethod.IsStatic)
                        {
                            var result = getMethod.Invoke(null, new object[] { key });
                            if (result is byte[] resultBytes) return resultBytes;
                        }
                    }
        ...
        public UsrExcelReportResponse Generate(UsrExcelReportRequest request)
        {
            try
            {
                ...
                // Try to get the file content immediately while still in same request context
                byte[] fileBytes = null;
                if (resultStr.StartsWith("ExportFilterKey_") || Guid.TryParse(resultStr, out _))
                {
                    fileBytes = TryGetFileFromCache(resultStr);
                }
                ...
                if (resultStr.StartsWith("ExportFilterKey_"))
                {
                    var response = new UsrExcelReportResponse
                    {
                        ...
                        fileName = "CommissionReport.xlsx"
                    };

                    if (fileBytes != null && fileBytes.Length > 0)
                    {
                        response.fileContent = Convert.ToBase64String(fileBytes);
                        response.message = $"Report generated successfully ({fileBytes.Length} bytes)";
                    }
                    else
                    {
                        response.message = "Report generated but file not found in cache (key: " + resultStr + ")";
                    }
        ...
        public Stream GetFile(string key)
        {
            try
            {
                byte[] fileBytes = TryGetFileFromCache(key);
